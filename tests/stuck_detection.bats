...
@test "stuck detection resets when a different story becomes the next incomplete" {
  ...
  # Create a mock that fails twice, then passes on third call
  # We'll use a counter file to track calls
  CALL_COUNTER="$TEST_DIR/call-count"
  echo "0" > "$CALL_COUNTER"

  # Create a custom mock that passes on 3rd call
...
if [ "$COUNT" -eq 3 ]; then
  # Pass the story on 3rd iteration
